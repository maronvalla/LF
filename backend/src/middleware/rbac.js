...
function requireAnyPermission(...requestedPermissions) {
  const permissionsToCheck = requestedPermissions.flat().filter(Boolean);
  return async (req, res, next) => {
    const userRole = req.user?.role;
    if (!userRole) return res.status(401).json({ ok: false, message: "No autenticado" });
    try {
      const permissions = await getPermissionsForRole(userRole);
      const allowed = permissionsToCheck.some((permission) =>
        hasPermissionInList(permissions, permission)
      );
      if (!allowed) {
        return res.status(403).json({ ok: false, message: "Sin permisos para esta accion" });
      }
...
module.exports = { requirePermission, requireAnyPermission, isAdmin };
